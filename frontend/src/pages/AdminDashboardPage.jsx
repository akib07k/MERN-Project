...
const AdminDashboardPage = () => {
  ...
  const { data: orders, isLoading: ordLoading, error: ordError, refetch: refetchOrders } = useGetOrdersQuery();
  ...
  useEffect(() => {
    if (tab === 'Products') refetchProducts();
    if (tab === 'Orders') refetchOrders();
    if (tab === 'Users') refetchUsers();
  }, [tab, refetchProducts, refetchOrders, refetchUsers]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
            ...
            <div className="flex justify-center gap-2 mt-6">
              {[...Array(productsData.pages).keys()].map((x) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${x + 1 === productsData.page
      ...
      {tab === 'Orders' && (
        <div>
          ...
          {ordLoading ? <Loader /> : ordError ? (
            <div className="bg-red-50 text-red-700 p-4 rounded-xl">
              {ordError?.data?.message || 'Error loading orders'}
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
      ...
      {tab === 'Users' && (
        <div>
          ...
          {usersLoading ? <Loader /> : usersError ? (
            <div className="bg-red-50 text-red-700 p-4 rounded-xl">
              {usersError?.data?.message || 'Error loading users'}
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
